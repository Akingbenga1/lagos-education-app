...
$(function () {
    ...
    var StudentListArray = $.map(StudentList, function (value, key) { return { value: value, data: key }; });
    console.log(StudentListArray);

     $('#autocomplete-custom-append').keyup(function() {
           //alert('Only students from the list can be choose.<br/> You must select the student first before you can proceed')
           $('#AutoButton, .AutoButtonRight').prop('disabled', true);
        });
     $('#autocomplete-custom-append2').keyup(function() {
           //alert('Only students from the list can be choose.<br/> You must select the student first before you can proceed')
           $('#AutoButton, .AutoButtonRight').prop('disabled', true);
        });
   ...
   /* $.mockjax({
    ...
    $('#autocomplete-custom-append').autocomplete({
        lookup: StudentListArray,
        appendTo: '#suggestions-container',
        onSelect: function (suggestion) {
            $('#StuId').val(suggestion.data);
